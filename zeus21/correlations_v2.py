...
class Correlations:
    ...
    def __init__(self, UserParams, Cosmo_Parameters, ClassCosmo):
        ...
        # CORRECTION (v2): the eta correlation function has been split into
        # parallel and perpendicular components (P_etapar and P_etaperp,
        # built in cosmology_v2.runclass). We carry both through and store
        # the corresponding windowed correlation matrices xiEtaPar_RR_CF
        # and xiEtaPerp_RR_CF.
        if Cosmo_Parameters.USE_RELATIVE_VELOCITIES == True:
            P_etapar_interp  = interp1d(ClassCosmo.pars['k_etapar'],  ClassCosmo.pars['P_etapar'],  bounds_error = False, fill_value = 0)
            P_etaperp_interp = interp1d(ClassCosmo.pars['k_etaperp'], ClassCosmo.pars['P_etaperp'], bounds_error = False, fill_value = 0)
            self._PkEtaParCF  = P_etapar_interp(self._klistCF)
            self._PkEtaPerpCF = P_etaperp_interp(self._klistCF)
            self.xiEtaPar_RR_CF  = self.get_xiEta_R1R2(Cosmo_Parameters, self._PkEtaParCF)
            self.xiEtaPerp_RR_CF = self.get_xiEta_R1R2(Cosmo_Parameters, self._PkEtaPerpCF)
        else:
            self._PkEtaParCF  = np.zeros_like(self._PklinCF)
            self._PkEtaPerpCF = np.zeros_like(self._PklinCF)
            self.xiEtaPar_RR_CF  = np.zeros_like(self.xi_RR_CF)
            self.xiEtaPerp_RR_CF = np.zeros_like(self.xi_RR_CF)
    def _WinTH(self,k,R):
        x = k * R
    ...
    def get_xiEta(self, Cosmo_Parameters, ClassCosmo):
        ...
        # CORRECTION (v2): unused diagnostic; report the parallel-component
        # correlation function for backward visual compatibility.
        rsEtaCF, xiEtaCF = self._xif(self._PkEtaParCF, extrap=False)

        return rsEtaCF, xiEtaCF

    def get_xiEta_R1R2(self, Cosmo_Parameters, _PkEta):
        "same as get_xiEta but smoothed over two different radii with Window"

        # CORRECTION (v2): generalized to take an explicit eta power-spectrum
        # array (_PkEta) so the same routine can build either the parallel or
        # perpendicular correlation matrix from the corresponding P_eta_{par,perp}.
        ...
        _PkEtaRR = np.array([[_PkEta]]) * windowR1 * windowR2
...
#            print("Computing Pop III-dependent power spectra")
        ...
        # CORRECTION (v2) NOTE: the linearized VCB-fluctuation terms below remain
        # commented out as in the baseline. If they are ever enabled, the single
        # '_PkEtaCF' contribution must be generalized to a par/perp combination
        # consistent with the corrected get_xi_Sum_2ExpEta. No functional change here.
...
#        print("Power Spectral Routine Done!")
    ...
    def get_xi_Sum_2ExpEta(self, xiEtapar, xiEtaperp, etaCoeff1, etaCoeff2):
        ...
        #
        # CORRECTION (v2): the four-term sum below originally used factors of
        #     (1 - K * xiEta)**(3/2)
        # for K = 6*bb*gg/((1+2*bb)*(1+2*gg)), 6*bb*kk/((1+2*bb)*(1+2*kk)),
        # 6*dd*gg/((1+2*dd)*(1+2*gg)), 6*dd*kk/((1+2*dd)*(1+2*kk)).
        # Each factor is now replaced by
        #     ((1 - K * xiEtapar) * (1 - K * xiEtaperp)**2)**(1/2)
        # which requires the parallel and perpendicular eta correlation
        # functions (built in cosmology_v2.runclass and propagated through
        # Correlations.xiEta{Par,Perp}_RR_CF).
        ...
        # CORRECTION (v2): factor replacement implemented term-by-term below.
        # Each (1 - K*xi)**(3/2) -> ((1 - K*xipar)*(1 - K*xiperp)**2)**(1/2).
    ...
    def get_all_corrs_III(self, User_Parameters, Cosmo_Parameters, Correlations, T21_coefficients):
        ...
        # CORRECTION (v2): build parallel and perpendicular eta correlation
        # matrices in place of the original isotropic corrEtaNL.
        corrEtaNLpar = Correlations.xiEtaPar_RR_CF[np.ix_(_iRnonlinear,_iRnonlinear)]
        ...
        corrEtaNLperp = Correlations.xiEtaPerp_RR_CF[np.ix_(_iRnonlinear,_iRnonlinear)]
        ...
        if Cosmo_Parameters.USE_RELATIVE_VELOCITIES == True:
            # CORRECTION (v2): pass parallel and perpendicular eta correlation matrices.
        ...
        # CORRECTION (v2): expand both par and perp matrices for the per-ir loop below.
        ...
        for ir in range(len(T21_coefficients.Rtabsmoo)):
            corrdNL = corrdNLBIG[:,:,:,:,ir]
            # CORRECTION (v2): use par/perp slices in place of the isotropic corrEtaNL.
