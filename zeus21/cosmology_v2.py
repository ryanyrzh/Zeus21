...
# CORRECTION (v2): use the corrected Correlations class which carries the
# par/perp-split eta correlation functions.
from .correlations_v2 import Correlations
...
def runclass(CosmologyIn):
    ...
    if CosmologyIn.USE_RELATIVE_VELOCITIES == True:
        ...
        # CORRECTION (v2): split the isotropic eta correlation function into
        # parallel and perpendicular components. The original code computed a
        # single P_eta from the integrand (6*psi0**2 + 3*psi2**2). The corrected
        # treatment carries two correlation functions:
        #   xi_eta_par   <- integrand 6*(psi0 + psi2)**2
        #   xi_eta_perp  <- integrand 6*(psi0 - psi2/2)**2
        # These feed the modified factor in get_xi_Sum_2ExpEta:
        #   ((1 - K*xiEtapar) * (1 - K*xiEtaperp)**2)**(1/2)
        # in place of the original (1 - K*xiEta)**(3/2).
        ...
        # NOTE (v2): the original 'k_eta'/'P_eta' keys are intentionally NOT
        # set here so that any stale consumer of the isotropic version fails
        # loudly rather than silently using the wrong correlation function.
